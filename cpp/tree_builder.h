...
std::unique_ptr<EvalNode> TreeBuilder<M, N>::BuildTree() {
  ...
  for (int i = 0; i < M * N; i++) {
    ...
    child->cell = i;
    auto score = DoAllDescents(i, 0, dict_, child);
    if (score > 0) {
...
template<int M, int N>
unsigned int TreeBuilder<M, N>::DoAllDescents(int idx, int length, Trie* t, EvalNode* node) {
  ...
  for (int j = 0; j < n; j++) {
    ...
    if (t->StartsWord(cc)) {
      ...
      child->cell = idx;
      child->letter = j;
      auto tscore = DoDFS(idx, length + (cc == kQ ? 2 : 1), t->Descend(cc), child);
      if (tscore > 0) {
...
unsigned int TreeBuilder<M, N>::DoDFS(int i, int length, Trie* t, EvalNode* node) {
  ...
  for (int j = 1; j <= n_neighbors; j++) {
    ...
    if ((used_ & (1<<idx)) == 0) {
      ...
      auto tscore = DoAllDescents(idx, length, t, neighbor);
      if (tscore > 0) {
        score += tscore;
